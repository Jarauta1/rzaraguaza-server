create or replace function rzaraguaza.create_asistencia(
	i_jugador_id INT,
	i_partido_id INT
)
returns void as $$
...
begin
	...
	if v_jugador_exists and v_partido_exists is true then
		insert into rzaraguaza.asistencia (jugador_id, partido_id)
...
create or replace function rzaraguaza.update_asistencia(
	i_asistencia_id INT,
	i_jugador_id INT,
	i_partido_id INT
)
returns void as $$
declare
	v_asistencia_exists boolean;
begin
	select into v_asistencia_exists exists (select 1 from rzaraguaza.asistencia a where a.asistencia_id = i_asistencia_id);
	if v_asistencia_exists then
		update rzaraguaza.asistencia ua
		set	jugador_id = i_jugador_id,
			partido_id = i_partido_id
		where ua.asistencia_id = i_asistencia_id;
	else
